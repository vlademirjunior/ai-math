...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        has_long_text = bool(
            re.search(r"<p|<div.*class=.*text|article|<span.*text", content, re.IGNORECASE)
        )
        has_form = bool(
            re.search(r"<form|<input|password|credit|card|payment", content, re.IGNORECASE)
        )
        ...
        if "button" in content.lower() and not re.search(
            r'primary|bg-primary|Button.*primary|variant=["\']primary', content, re.IGNORECASE
        ):
            ...
            if nav_content and len(nav_content) > 2:
                ...
                if not any(
                    ...
                    self.warnings.append(
                        f"[Serial Position] {filename}: Last nav item may not be important. Place key actions at start/end."
                    )
        ...
        if has_hero:
            ...
            if not has_visual_interest and not re.search(r"background:|bg-", content):
                self.warnings.append(
                    f"[Visceral] {filename}: Hero section lacks visual appeal. Consider gradients or subtle animations."
                )
        ...
        if "onClick" in content or "@click" in content or "onclick" in content:
            has_feedback = re.search(
                r"transition|animate|hover:|focus:|disabled|loading|spinner", content, re.IGNORECASE
            )
            has_state_change = re.search(r"setState|useState|disabled|loading", content)

            if not has_feedback and not has_state_change:
                self.warnings.append(
                    f"[Behavioral] {filename}: Interactive elements lack immediate feedback. Add hover/focus/disabled states."
                )
        ...
        if has_long_text and not has_reflective:
            self.warnings.append(
                f"[Reflective] {filename}: Long-form content without brand story/values. Add 'About' or 'Why We Exist' section."
            )
        ...
        social_proof = re.findall(
            r"review|testimonial|rating|star|trust|trusted by|customer|logo", content, re.IGNORECASE
        )
        if len(social_proof) > 0:
            ...
            if has_long_text:
                self.warnings.append(
                    f"[Trust] {filename}: No social proof detected. Consider adding testimonials, ratings, or 'Trusted by' logos."
                )
        ...
        if complex_elements > 5:
            ...
            if not has_progressive:
                self.warnings.append(
                    f"[Cognitive Load] {filename}: Many form elements without progressive disclosure. Consider accordion, tabs, or 'Advanced' toggle."
                )
        ...
        if has_many_colors and has_many_borders:
            self.warnings.append(
                f"[Cognitive Load] {filename}: High visual noise detected. Many colors and borders increase cognitive load."
            )
        ...
        if has_form:
            ...
            if radio_inputs > 0 and not has_defaults:
                self.warnings.append(
                    f"[Persuasion] {filename}: Radio buttons without default selection. Pre-select recommended option."
                )
        ...
        if has_social:
            ...
            if not has_count:
                self.warnings.append(
                    f"[Persuasion] {filename}: Social proof without specific numbers. Use 'Join 10,000+' format."
                )
        ...
        if len(font_families) > 3:
            self.issues.append(
                f"[Typography] {filename}: {len(font_families)} font families detected. Limit to 2-3 for cohesion."
            )
        ...
        if re.search(r"<h[1-6]|text-(?:xl|2xl|3xl|4xl|5xl|6xl)", content, re.IGNORECASE):
            ...
            for lh in line_heights:
                if float(lh) > 1.5:
                    self.warnings.append(
                        f"[Typography] {filename}: Heading has line-height {lh} (>1.3). Headings should be tighter (1.1-1.3)."
                    )
        ...
        if headings:
            ...
            for i in range(len(headings) - 1):
                ...
                if next_h > curr + 1:
                    self.warnings.append(
                        f"[Typography] {filename}: Skipped heading level (h{curr} -> h{next_h}). Maintain sequential hierarchy."
                    )

            # Check if h1 exists for main content
            if "h1" not in [h.lower() for h in headings] and has_long_text:
                self.warnings.append(
                    f"[Typography] {filename}: No h1 found. Each page should have one primary heading."
                )
        ...
        if len(size_values) > 2:
            ...
            for ratio in ratios[:3]:  # Check first 3 ratios
                if not any(abs(ratio - cr) < 0.05 for cr in common_ratios):
                    self.warnings.append(
                        f"[Typography] {filename}: Font sizes may not follow modular scale (ratio: {ratio:.2f}). Consider consistent ratio like 1.25 (Major Third)."
                    )
                    break
        ...
        for p in paragraphs:
            ...
            if word_count > 100:  # ~5-6 lines
                self.warnings.append(
                    f"[Typography] {filename}: Long paragraph detected ({word_count} words). Break into 3-4 line chunks for readability."
                )
        ...
        if len(paragraphs) > 5:
            ...
            if subheadings == 0:
                self.warnings.append(
                    f"[Typography] {filename}: Long content without subheadings. Add h2/h3 to break up text."
                )
        ...
        if re.search(r"@keyframes|transition:", content):
            ...
            if not re.search(r"prefers-reduced-motion", content):
                self.warnings.append(
                    f"[Accessibility] {filename}: Animations found without prefers-reduced-motion check"
                )
        ...
        if shadow_count > 0:
            ...
            if shadow_count >= 3 and len(shadow_opacities) > 0:
                ...
                if unique_opacities < 2:
                    self.warnings.append(
                        f"[Visual] {filename}: All shadows at same opacity level. Vary shadow intensity for elevation hierarchy."
                    )

        # --- 3.3 GRADIENT CHECKS ---
        # Check for gradient usage
        has_gradient = bool(
            re.search(r"gradient|linear-gradient|radial-gradient|conic-gradient", content)
        )
        if has_gradient:
            ...
            if gradient_count > 5:
                self.warnings.append(
                    f"[Visual] {filename}: Many gradients detected ({gradient_count}). Ensure this serves purpose, not decoration."
                )
        else:
            # Check if hero section exists without gradient
            if has_hero and not re.search(r"background:|bg-", content):
                self.warnings.append(
                    f"[Visual] {filename}: Hero section without visual interest. Consider gradient for depth."
                )
        ...
        if has_border:
            ...
            if border_count > 8:
                self.warnings.append(
                    f"[Visual] {filename}: Many border declarations ({border_count}). Simplify for cleaner look."
                )
        ...
        for ts in text_shadows:
            ...
            if "," in ts:
                self.warnings.append(
                    f"[Visual] {filename}: Text glow effect detected. Ensure readability is maintained."
                )
        ...
        if len(glow_shadows) > 2:
            self.warnings.append(
                f"[Visual] {filename}: Multiple glow effects detected. Use sparingly for emphasis only."
            )
        ...
        if has_images and has_long_text:
            has_overlay = bool(
                re.search(r"overlay|rgba\(0|gradient.*transparent|::after|::before", content)
            )
            if not has_overlay:
                self.warnings.append(
        ...
        if re.search(r"will-change:", content):
            ...
            for prop in will_change_props:
                ...
                if prop in [
                    ...
                    self.issues.append(
                        f"[Performance] {filename}: will-change on '{prop}' (layout property). Use only for transform/opacity."
                    )
        ...
        if will_change_count > 3:
            self.warnings.append(
                f"[Performance] {filename}: Many will-change declarations ({will_change_count}). Use sparingly, only for heavy animations."
            )
        ...
        if effect_count > 10:
            self.warnings.append(
                f"[Visual] {filename}: Many visual effects ({effect_count}). Ensure effects serve purpose, not decoration."
            )

        # Check for static/flat design (no depth)
        if has_long_text and effect_count == 0:
            self.warnings.append(
                f"[Visual] {filename}: Flat design with no depth. Consider shadows or subtle gradients for hierarchy."
            )
        ...
        for purple in purple_hexes:
            if purple.lower() in content.lower():
                self.issues.append(
                    f"[Color] {filename}: PURPLE DETECTED ('{purple}'). Banned by Maestro rules. Use Teal/Cyan/Emerald instead."
                )
                break
        ...
        if total_colors > 3:
            ...
            if len(bg_declarations) > 0 and len(text_declarations) > 0:
                ...
                if len(unique_hexes) > 5:
                    self.warnings.append(
                        f"[Color] {filename}: {len(unique_hexes)} distinct colors. Consider 60-30-10 rule: dominant (60%), secondary (30%), accent (10%)."
                    )
        ...
        if len(hsl_matches) >= 3:
            ...
            if hue_range < 10:
                self.warnings.append(
                    f"[Color] {filename}: Monochromatic palette detected (hue variance: {hue_range}deg). Ensure adequate contrast."
                )
        ...
        light_bg_light_text = bool(
            re.search(r"bg-(?:gray|slate|zinc)-50|bg-white.*text-(?:gray|slate)-[12]", content)
        )
        dark_bg_dark_text = bool(
            re.search(r"bg-(?:gray|slate|zinct)-9|bg-black.*text-(?:gray|slate)-[89]", content)
        )
        ...
        has_food_context = bool(
            re.search(r"restaurant|food|cooking|recipe|menu|dish|meal", content, re.IGNORECASE)
        )
        ...
        durations = re.findall(
            r"(?:duration|animation-duration|transition-duration):\s*([\d.]+)(s|ms)", content
        )
        for duration, unit in durations:
            duration_ms = float(duration) * (1000 if unit == "s" else 1)
            if duration_ms < 50:
                self.warnings.append(
                    f"[Animation] {filename}: Very fast animation ({duration}{unit}). Minimum 50ms for visibility."
                )
            elif duration_ms > 1000 and "transition" in content.lower():
                self.warnings.append(
                    f"[Animation] {filename}: Long transition ({duration}{unit}). Transitions should be 100-300ms for responsiveness."
                )
        ...
        if re.search(r"ease-in\s+.*entry|fade-in.*ease-in", content):
            self.warnings.append(
                f"[Animation] {filename}: Entry animation with ease-in. Entry should use ease-out for snappy feel."
            )
        if re.search(r"ease-out\s+.*exit|fade-out.*ease-out", content):
            self.warnings.append(
                f"[Animation] {filename}: Exit animation with ease-out. Exit should use ease-in for natural feel."
            )
        ...
        if interactive_elements > 2 and not has_hover_focus:
            self.warnings.append(
                f"[Animation] {filename}: Interactive elements without hover/focus states. Add micro-interactions for feedback."
            )
        ...
        if has_lottie:
            ...
            has_lottie_fallback = bool(
                re.search(r"prefers-reduced-motion.*lottie|lottie.*isPaused|lottie.*stop", content)
            )
            if not has_lottie_fallback:
                self.warnings.append(
                    f"[Motion] {filename}: Lottie animation without reduced-motion fallback. Add pause/stop for accessibility."
                )
        ...
        if has_3d_transform:
            ...
            self.warnings.append(
                f"[Motion] {filename}: 3D transforms detected. Test on mobile; can impact performance on low-end devices."
            )

        # 6.5 Particle Effect Warnings
        # Check for canvas/WebGL particle systems
        has_particles = bool(
            re.search(r"particle|canvas.*loop|requestAnimationFrame.*draw|Three\.js", content)
        )
        if has_particles:
            self.warnings.append(
        ...
        has_scroll_driven = bool(
            re.search(r"IntersectionObserver.*animate|scroll.*progress|view-timeline", content)
        )
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [
                d for d in dirs if d not in {"node_modules", ".git", "dist", "build", ".next"}
            ]
            for file in files:
